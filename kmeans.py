...
def perform_clustering(descriptors, k=5):
...
def find_similar_in_cluster(query_image_path, cluster, cluster_image_paths, descriptors, N=5):
    ...
    for idx in cluster:
        ...
        distance = np.linalg.norm(query_descriptor - base_descriptor)
...
def evaluate_system_with_clustering(query_images, base_directory, k=5, N=5):
    ...
    print(f"F1-Score moyen: {average_f1_score:.4f}")
